...
import org.grical.android.EventRequest;
...
public class AdvancedMapViewer extends MapActivity {
	...
	private void createOldOverlay() {
		...
		this.circleOverlay = new ArrayCircleOverlay(circleDefaultPaintFill,
		...
		this.circleOverlay.addCircle(circle1);
		this.circleOverlay.addCircle(circle2);
	...
	protected void createOverlay() {
		...
		for (int i = 0; i < length; i++) {
			try {
				...
				if (fields.has("coordinates") && fields.has("title") && fields.has("upcoming")) {
					...
					if (latlong.length == 2) {
						...
						OverlayItem item = new OverlayItem(geopoint, upcoming, title);
