...
public class POIsProvider extends ContentProvider {
	...
	@Override
	public synchronized int bulkInsert(Uri uri, ContentValues[] valuesArray) {
	...
	private String buildDistanceQuery(double latitude, double longitude,
		...
		// @formatter:off
			a.append("(").append(sin_lat_rad).append("*\"sin_lat_rad\"+").append(cos_lat_rad).append("*\"cos_lat_rad\"*");
			a.append("(").append(cos_lon_rad).append("*\"cos_lon_rad\"+").append(sin_lon_rad).append("*\"sin_lon_rad\"))");
		...
		// @formatter:off
			 ...
			 * ))
			 * AS "distance_acos" FROM "pois"
			 * WHERE
			 * 		category_id=8 OR category_id=10 
			...
			/*
			 * return 'SELECT *, 
			 * 			(%(sin_lat_rad)f * "sin_lat_rad"
			 *   		 %(cos_lat_rad)f * "cos_lat_rad" * (%(cos_lon_rad)f *
			 *			 "cos_lon_rad" + %(sin_lon_rad)f * "sin_lon_rad")) 
			 * AS "distance_acos" FROM "pois" 
			 * GROUP BY "id" HAVING "distance_acos" < 1.25 
			 * ORDER BY "distance_acos" 
			 * DESC' % {'sin_lat_rad':sin_lat_rad, 
			 * 			"cos_lat_rad": cos_lat_rad, 'sin_lon_rad':
